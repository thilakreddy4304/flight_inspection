...
import InspectionStage3 from './InspectionStage3';
...
interface InspectionStage2Props {
  flightId: string;
...
const InspectionStage2: React.FC<InspectionStage2Props> = ({
  flightId,
  inspectionType,
  inspectionName,
  flightData
        ...
        <ContentContainer>
          <StepsContainer>
            {processingSteps.map((section, index) => (
              <StepSection key={index} isActive={index === 1}>
                <StepTitle data-number={index + 1} isActive={index === 1}>{section.title}</StepTitle>
                <StepList>
                  {section.steps.map((step, stepIndex) => (
                    <StepItem key={stepIndex} isActive={index === 1}>{step}</StepItem>
                  ))}
                </StepList>
              </StepSection>
            ))}
          ...
          <ImageContainer>
            <StageIndicator>
              <StageTitle>Stage 2: Inspection...60%</StageTitle>
              <StageText>Capturing data on zones 12/24.</StageText>
...
export default InspectionStage2; 